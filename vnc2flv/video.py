...
class VideoSink(object):
    ...
    def update_screen_solidrect(self, (x, y), (w, h), data):
        if self.debug:
            print >>sys.stderr, 'update_screen_solidrect: %dx%d at (%d,%d), color=%r' % (w,h,x,y, color)
        return

    def update_screen_copyrect(self, (xdst, ydst), (xsrc, ysrc), (w, h)):
        if self.debug:
            print >>sys.stderr, 'update_screen_copyrect: %dx%d at (%d,%d) to (%d,%d)' % (w,h,x,y,xdst,ydst)
        return
...
class FLVVideoSink(VideoSink):
    ...
    def update_screen_solidrect(self, (x, y), (w, h), data):
        self.update_screen_rgbabits((x,y),(w,h),data*(int(w)*int(h)))
        return

    def update_screen_copyrect(self, (xdst, ydst), (xsrc, ysrc), (w, h)):
        data = self.screen.get_rgba(xsrc, ysrc, w, h)
        self.update_screen_rgbabits((xdst,ydst),(w,h),data)
        return
    ...
    def flush(self, t):
        ...
        while 1:
            ...
            if t < timestamp: 
                sleep(float(max((timestamp-t)/2,20))/1000.0)
                break
