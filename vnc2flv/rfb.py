...
class RFBProxy(object):
    ...
    def __init__(self, sink, pwdcache=None, preferred_encoding=(5,1,0), debug=0):
        self.sink = sink
    ...
    def encraw(self):
        (w,h) = self.rectsize
        return (w*h*self.bytesperpixel, self.encraw_1)
    ...
    def enccopy_1(self, data):
        (xsrc,ysrc) = unpack(">HH", data)
        self.sink.update_screen_copyrect(self.rectpos, (xsrc,ysrc), self.rectsize)
        return self.framerect()
    ...
    def enchex(self):
        ...
        self.fgcolor = None;
        self.bgcolor = None;
        return self.enchex_1()

    def enchex_1(self):
        (w,h) = self.rectsize
        self.hexy = self.hexy + 16
        self.hexx = -16
        if self.hexy < h:
            return self.enchex_2()
        else:
            return self.framerect()

    def enchex_2(self):
        (w,h) = self.rectsize        
        self.hexx = self.hexx + 16
        if self.hexx < w:
            return (1, self.enchex_3)
        else:
            return self.enchex_1()

    def enchex_3(self, data):
        (w,h) = self.rectsize
        self.hexw = min(w-self.hexx, 16)
        self.hexh = min(h-self.hexy, 16)
        (self.c,) = unpack('>B', data)
        if self.debug:
            print >>sys.stderr, ' enchex_3: at x=%d,y=%d,w=%d,h=%d,c=%d' % (self.hexx,self.hexy,self.hexw,self.hexh,self.c)
        return self.enchex_4()

    def enchex_4(self):
        if self.c & 1:
            self.c = 0
            # Raw
            return (self.hexw*self.hexh*self.bytesperpixel, self.enchex_raw)
        if self.c & 2:
            self.c = self.c ^ 2
            # bgcolor
            return (self.bytesperpixel, self.enchex_bg)
        if self.c & 4:
            self.c = self.c ^ 4
            # fgcolor
            return (self.bytesperpixel, self.enchex_fg)
        # Set bgcolor
        (x0,y0) = self.rectpos
        self.sink.update_screen_solidrect((x0+self.hexx,y0+self.hexy), (self.hexw, self.hexh), self.bgcolor)
        if not self.c & 8:
            # Solid
            self.c = 0
            return self.enchex_2()
        return (1, self.enchex_5)

    def enchex_5(self, data):
        (self.nsubrects,) = unpack(">B", data)
        if self.c & 16:
            self.c = self.c ^ 16
            # SubrectsColoured
            return self.enchex_src()
        else:
            # NoSubrectsColoured
            return self.enchex_nsrc()

    def enchex_src(self):
        if self.nsubrects:
            self.nsubrects -= 1
            return (self.bytesperpixel+2, self.enchex_src_1)
        else:
            return self.enchex_2()

    def enchex_src_1(self, data):
        (x0,y0) = self.rectpos
        (color, xy, wh) = unpack('>lBB', data)
        color = pack(">l",color)
        self.sink.update_screen_solidrect((x0+self.hexx+(xy>>4),y0+self.hexy+(xy&15)),((wh>>4)+1, (wh&15)+1), color)
        return self.enchex_src()

    def enchex_nsrc(self):
        if self.nsubrects:
            self.nsubrects -= 1
            return (2, self.enchex_nsrc_1)
        else:
            return self.enchex_2()

    def enchex_nsrc_1(self, data):
        (x0,y0) = self.rectpos
        (xy, wh) = unpack('>BB', data)
        self.sink.update_screen_solidrect((x0+self.hexx+(xy>>4),y0+self.hexy+(xy&15)),((wh>>4)+1, (wh&15)+1), self.fgcolor)
        return self.enchex_nsrc()

    def enchex_bg(self, data):
        self.bgcolor = data;
        return self.enchex_4()

    def enchex_fg(self, data):
        self.fgcolor = data;
        return self.enchex_4()

    def enchex_raw(self, data):
        (x0,y0) = self.rectpos
        self.sink.update_screen_rgbabits((x0+self.hexx,y0+self.hexy), (self.hexw,self.hexh), data)
        return self.enchex_2()
...
class RFBNetworkClient(RFBProxy):
    ...
    def __init__(self, host, port, sink, timeout=50, bufsiz=65536,
                 pwdcache=None, preferred_encoding=(5,1,0), debug=0):
