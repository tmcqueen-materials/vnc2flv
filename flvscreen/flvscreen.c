...
FlvScreen_init(PyFlvScreen* self, PyObject* args, PyObject* kwargs)
{
    ...
    self->tmpbuf = PyMem_Malloc(self->pix_width * self->pix_height * sizeof(RGBAPixel));
    if (self->tmpbuf == NULL) return -1;
    memset(self->tmpbuf, 0, self->pix_width * self->pix_height * sizeof(RGBAPixel));
...
/* FlvScreen.get_rgba(x,y,w,h)
 *   returns a rectange of the screen (for copyrect support).
 */
static PyObject*
FlvScreen_get_rgba(PyFlvScreen* self, PyObject* args)
{
    PyObject* result;
    int x0,y0,y,w,h,wadj;

    if (!PyArg_ParseTuple(args, "iiii", &x0, &y0, &w, &h)) {
        return NULL;
    }
    if (x0 < 0 || y0 < 0 || w < 1 || h < 1) {
        PyErr_SetString(PyExc_FlvError, "out of range");
        return NULL;
    }

    memset(self->tmpbuf, 0, w * h * sizeof(RGBAPixel));
    wadj = self->blk_width*self->blk_size-x0;
    if (w < wadj) wadj = w;

    for (y = y0; y < y0+h && y < self->blk_height*self->blk_size; y++) {
        memcpy(self->tmpbuf + (y-y0)*w,
               self->pixels + y*self->blk_width*self->blk_size + x0, wadj*sizeof(RGBAPixel));
    }

    result = PyString_FromStringAndSize((char*)self->tmpbuf,
                                        w * h * sizeof(RGBAPixel));
...
static PyMethodDef FlvScreen_methods[] = {
    ...
    { "get_rgba", (PyCFunction)FlvScreen_get_rgba, METH_VARARGS,
      "get_rgba"
    },
